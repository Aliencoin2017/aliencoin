...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx)
{
            ...
            BOOST_FOREACH(const CTxOut& txout, wtx.vout)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    if (wallet->HaveKey(ckidD) && (wallet->mapAddressBook[ckidD].empty() || !wallet->mapAddressBook[ckidD].compare(0, sAnonPrefix.length(), sAnonPrefix) == 0))
                    {
                        ...
                        strHTML += "<br><br>";
                        strHTML += "<b>" + tr("To") + ":</b> <a href='"+explorer+"/Address/index.php?Currency=ALIEN&Address=";
    ...
    if (wtx.mapValue.count("to") && !wtx.mapValue["to"].empty())
    {
        ...
        strHTML += "<b>" + tr("To") + ":</b> <a href='"+explorer + "/Address/index.php?Currency=ALIEN&Address=" + GUIUtil::HtmlEscape(strAddress) + "' target='_blank'>";
    ...
    if (wtx.IsCoinBase() && nCredit == 0)
    {
        ...
        if (wtx.IsInMainChain())
            strHTML += BitcoinUnits::formatWithUnit(BitcoinUnits::ALIEN, nUnmatured)+ " (" + tr("matures in %n more block(s)", "", wtx.GetBlocksToMaturity()) + ")";
        else
    ...
    if (nNet > 0)
    {
        ...
        // Credit
        //
        strHTML += "<b>" + tr("Credit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::ALIEN, nNet);
        strHTML += "<br><br>";
    } else
    {
        ...
        if (fAllFromMe)
        {
            ...
            BOOST_FOREACH(const CTxOut& txout, wtx.vout)
            {
                ...
                if (!wtx.mapValue.count("to") || wtx.mapValue["to"].empty())
                {
                    ...
                    {
                        strHTML += "<b>" + tr("To") + ":</b> <a href='"+explorer+"/Address/index.php?Currency=ALIEN&Address="+GUIUtil::HtmlEscape(CBitcoinAddress(address).ToString())+"' target='_blank'>";
                ...
                strHTML += "<b>" + tr("Debit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::ALIEN, -txout.nValue);
                strHTML += "<br><br>";
            }
            ...
            if (fAllToMe)
            {
                ...
                strHTML += "<b>" + tr("Debit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::ALIEN, -nValue);
                strHTML += "<br><br>";
                strHTML += "<b>" + tr("Credit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::ALIEN, nValue);
                strHTML += "<br><br>";
            }

            int64_t nTxFee = nDebit - wtx.GetValueOut();
            if (nTxFee > 0)
                strHTML += "<b>" + tr("Transaction fee") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::ALIEN, -nTxFee);
                strHTML += "<br><br>";
        } else
        {
            //
            ...
            BOOST_FOREACH(const CTxIn& txin, wtx.vin)
            {
                ...
                if (wallet->IsMine(txin))
                    strHTML += "<b>" + tr("Debit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::ALIEN, -wallet->GetDebit(txin)) + "<br><br>";
            };

            BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                if (wallet->IsMine(txout))
                    strHTML += "<b>" + tr("Credit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::ALIEN, wallet->GetCredit(txout)) + "<br><br>";
        }
    }

    strHTML += "<b>" + tr("Net amount") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::ALIEN, nNet, true) + "<br><br>";
    ...
    strHTML += "<b>" + tr("Transaction ID") + ":</b> <a href='"+explorer+"/Transaction/index.php?Currency=ALIEN&Transaction=" + txid + "' target='_blank'>" + txid + "</a><br>";
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
            ...
            if (txdb.ReadDiskTx(prevout.hash, prev))
            {
                if (prevout.n < prev.vout.size())
                {
                    ...
                    {
                        strHTML +="<a href='"+explorer+"/Address/index.php?Currency=ALIEN&Address="+QString::fromStdString(CBitcoinAddress(address).ToString())+"' target='_blank'>";
                        if (wallet->mapAddressBook.count(address) && !wallet->mapAddressBook[address].empty())
                            strHTML += GUIUtil::HtmlEscape(wallet->mapAddressBook[address]) + " ";
                        strHTML += QString::fromStdString(CBitcoinAddress(address).ToString()) + "</a>";
                    }
                    strHTML = strHTML + " " + tr("Amount") + "=" + BitcoinUnits::formatWithUnit(BitcoinUnits::ALIEN, vout.nValue);
