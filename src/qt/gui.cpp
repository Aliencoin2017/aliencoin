// Copyright (c) 2014 The Aliencoin developers
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("Aliencoin") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/aliencoin"));
    setWindowIcon(QIcon(":icons/aliencoin"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/aliencoin/index.html");
#else
    QFile html("/opt/aliencoin/index.html");
#endif
...
void GUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("Aliencoin") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/aliencoin_testnet"));
            setWindowIcon(QIcon(":icons/aliencoin_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/aliencoin_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Aliencoin client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/aliencoin_testnet"));
...
void GUI::createTrayIcon()
{
    ...
    trayIcon->setIcon(QIcon(":/icons/aliencoin"));
...
void GUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to Aliencoin network", "", count));
}
    ...
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          ...
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::ALIEN, nFeeRequired));
