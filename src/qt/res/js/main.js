...
var qrcode = new QRCode("qrcode", {colorDark:'#4A3132', colorLight: '#ffffff', correctLevel: QRCode.CorrectLevel.H, width: 220, height: 220,});
    ...
    check: function(field)
    {
        ...
        for (var i = 0, len = value.length; i < len; ++i)
            if (base58.base58Chars.indexOf(value[i]) == -1) {
                el.css("background", "#555555").css("color", "white");
                return false;
            }
...
var unit = {
    ...
    setType: function(type) {
        ...
        switch(type) {
            ...
            case 2:
                this.name = "uALIEN",
                this.display = "&micro;ALIEN";
                break;

            case 3:
                this.name    = "sALIEN",
                this.display = "Shi-Aliencoins";
                break;

            default:
                this.name = this.display = "ALIEN";
        }
...
var overviewPage = {
    init: function() {
        ...
        menu = [{
                 ...
                 {
                     name: ' About&nbsp;Aliencoin...',
                     img: 'qrc:///icons/aliencoin',
                     fun: function () {
    ...
    $("#recipients").append((
        ...
        +  '<div id="recipient[count]" class="recipient"> \
            <div class="flex-right"> \
                ...
                <input id="pay_to[count]" class="pay_to input_box" data-title="The address to send the payment to  (e.g. XDN6TyyoWuXPJYaVtpMrRYRE5sxDtkBkhH)" placeholder="Enter a Aliencoin address (e.g. XDN6TyyoWuXPJYaVtpMrRYRE5sxDtkBkhH)" maxlength="128" oninput="base58.check(this);" onchange="$(\'#label[count]\').val(bridge.getAddressLabel(this.value));"/> \
            ...
            <div class="flex-left"> \
                ...
                <select id="unit[count]" class="unit button is-inverse has-fixed-icon"  style="margin-left:10px; height:43px; width:100px;" onchange="unit.format(\'#amount[count]\', $(this).val());"> \
                    <option value="0" title="Aliencoin"                    ' + (unit.type == 0 ? "selected" : "") + '>ALIEN</option> \
                    <option value="1" title="Milli-Aliencoin (1 / 1000)"   ' + (unit.type == 1 ? "selected" : "") + '>mALIEN</option> \
                    <option value="2" title="Micro-Aliencoin (1 / 1000000)"' + (unit.type == 2 ? "selected" : "") + '>&micro;ALIEN</option> \
                    <option value="3" title="Shi-Aliencoins (1 / 100000000)" ' + (unit.type == 3 ? "selected" : "") + '>sALIEN</option> \
                </select> \
            </div> \
