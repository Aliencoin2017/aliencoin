...
bool UIBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    ...
    {
        int inputType; // 0 ALIEN, 1 Aliencoin
        switch(rcp.txnTypeInd)
        {
            case TXT_ALIEN_TO_ALIEN:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ALIEN, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_ALIEN_TO_ANON:
                formatted.append(tr("<b>%1</b> to TOKEN %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ALIEN, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b>, ring size %2 to TOKEN %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ALIEN, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ALIEN:
                formatted.append(tr("<b>%1</b>, ring size %2 to ALIEN %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ALIEN, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ALIEN, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_AmountWithFeeExceedsTokenBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your TOKEN balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::ALIEN, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
QVariantMap UIBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with Aliencoin prefix.");
            return result;
        }
