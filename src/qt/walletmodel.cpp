...
WalletModel::SendCoinsReturn WalletModel::sendCoinsAnon(const QList<SendCoinsRecipient> &recipients, const CCoinControl *coinControl)
{
    ...
    if (vNodes.empty())
        return SendCoinsReturn(SCR_ErrorWithMsg, 0, QString::fromStdString("Aliencoin is not connected!"));
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            if (rcp.txnTypeInd == TXT_ALIEN_TO_ALIEN
                || rcp.txnTypeInd == TXT_ANON_TO_ALIEN)
            {
                // -- out alien
                std::string sError;
