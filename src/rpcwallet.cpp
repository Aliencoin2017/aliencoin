...
Value getnewextaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "Returns a new Aliencoin ext address for receiving payments."
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "Returns the current Aliencoin stealth address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Aliencoin address");
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw std::runtime_error(
            "sendtoaddress <aliencoinaddress> <amount> [comment] [comment-to] [narration]\n" // Exchanges use the comments internally...
            "sendtoaddress <aliencoinaddress> <amount> [narration]\n"
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw std::runtime_error(
            "sendfrom <fromaccount> <toaliencoinaddress> <amount> [minconf=1] [comment] [comment-to] [narration] \n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    else if(!bHasStealth)
    {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Aliencoin address: ")+s.name_);
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; Aliencoin server stopping, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw std::runtime_error(
            "validatepubkey <aliencoinpubkey>\n"
            "Return information about <aliencoinpubkey>.");
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "Returns a new Aliencoin stealth address for receiving payments anonymously."
            + HelpRequiringPassphrase());
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    if (!sxAddr.SetEncoded(sEncoded))
        throw std::runtime_error("Invalid Aliencoin stealth address.");
...
Value sendalientotoken(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw std::runtime_error(
            "sendalientotoken <stealth_address> <amount> [narration] [comment] [comment-to]\n"
    ...
    std::string sError;
    if (!pwalletMain->SendALIENToToken(sxAddr, nAmount, sNarr, wtx, sError))
    {
        LogPrintf("SendALIENToToken failed %s\n", sError.c_str());
        throw JSONRPCError(RPC_WALLET_ERROR, sError);
    };
    return wtx.GetHash().GetHex();
}
...
Value sendtokentoalien(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw std::runtime_error(
            "sendtokentoalien <stealth_address> <amount> <ring_size> [narration] [comment] [comment-to]\n"
            "<amount> is a real number and is rounded to the nearest 0.000001\n"
            "<ring_size> is a number of outputs of the same amount to include in the signature"
            + HelpRequiringPassphrase());
    ...
    std::string sError;
    if (!pwalletMain->SendTokenToALIEN(sxAddr, nAmount, nRingSize, sNarr, wtx, sError))
    {
        LogPrintf("SendTokenToALIEN failed %s\n", sError.c_str());
        throw JSONRPCError(RPC_WALLET_ERROR, sError);
    };
    return wtx.GetHash().GetHex();
}
