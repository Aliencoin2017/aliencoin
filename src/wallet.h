...
public:
    ...
    bool SendALIENToToken(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    bool SendTokenToALIEN(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nALIEN, int64_t& nToken) const
    {
        nALIEN = 0;
        nToken = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                nALIEN += GetCredit(txout);
            if (!MoneyRange(nALIEN)
                || !MoneyRange(nToken))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
        return true;
...
public:
    ...
    mutable int64_t nCredALIENCached;
    ...
    bool GetCredit(int64_t& nCredALIEN, int64_t& nCredToken, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredALIEN = nCredToken = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredALIENCached = 0;
            nCredTokenCached = 0;
            pwallet->GetCredit(*this, nCredALIENCached, nCredTokenCached);
            fCreditSplitCached = true;
        };
        
        nCredALIEN = nCredALIENCached;
